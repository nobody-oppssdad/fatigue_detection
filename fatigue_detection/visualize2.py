...
CONFIG = {
    ...
    "batch_save_dir": "batch_results",
    "fatigue_frame_thresh": 3,  # 连续3帧疲劳才触发报警
    "fps_smooth_window": 5,  # FPS滑动平均窗口
    "use_fp16": True,  # 保留FP16提速，但修复NMS兼容
    "batch_sleep_ms": 20  # 批量每张图间隔，释放UI
}

# 创建保存目录
os.makedirs(CONFIG["save_dir"], exist_ok=True)
os.makedirs(CONFIG["batch_save_dir"], exist_ok=True)
...
class DetectorThread(QThread):
    ...
    # 新增：批量检测信号
    batch_progress_signal = pyqtSignal(int, int)
    batch_finish_signal = pyqtSignal(str)
    ...
    def batch_detect_images(self, folder_path):
        """批量图片检测【修复UI卡死】"""
        try:
            self.running = True
            # 获取文件夹内所有图片
            img_exts = ('.jpg', '.jpeg', '.png', '.bmp', '.JPG', '.PNG')
            img_list = [f for f in os.listdir(folder_path) if f.endswith(img_exts)]
            total = len(img_list)
            if total == 0:
                self.batch_finish_signal.emit("文件夹内无图片！")
                return
            ...
            for idx, img_name in enumerate(img_list):
                ...
                frame = cv2.imread(img_path)
                if frame is None:
                    self.batch_progress_signal.emit(idx + 1, total)
                    QThread.msleep(CONFIG["batch_sleep_ms"])
                    continue

                # 预处理 + 推理
                frame_input = cv2.resize(frame, CONFIG["img_size"])
                img_pil = Image.fromarray(cv2.cvtColor(frame_input, cv2.COLOR_BGR2RGB))
                img_tensor, _ = self.transform(img_pil, None)
                if self.device.type == "cuda" and CONFIG["use_fp16"]:
                    img_tensor = img_tensor.half()
                ...
                fatigue_level = self._judge_fatigue_level(labels) if len(boxes) > 0 else "normal"
                res_frame = self._draw_results(frame, boxes, scores, labels, fatigue_level, frame.shape[:2])

                # 保存到批量结果目录
                save_path = os.path.join(CONFIG["batch_save_dir"], img_name)
            ...
            if self.running:
                self.batch_finish_signal.emit(f"✅ 批量完成：共处理 {total} 张图片")
            else:
                self.batch_finish_signal.emit("⚠️ 批量任务已手动终止")

        except Exception as e:
            self.batch_finish_signal.emit(f"❌ 批量异常：{str(e)}")

    def run(self):
        """视频/摄像头检测（核心修复FP16+NMS兼容）"""
        # 批量模式
        if self.batch_folder:
            self.batch_detect_images(self.batch_folder)
            self.batch_folder = None
            return
...
class FatigueDetectionUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 新增：批量识别按钮
        self.batch_btn = QPushButton("批量识别文件夹")
    ...
    def batch_detect(self):
        ...
        self.stop_detection()
        self.save_label.setText("正在批量处理...")

        self.detector_thread = DetectorThread(self.checkpoint_path, device=self.device)
        self.detector_thread.batch_folder = folder
        self.detector_thread.batch_progress_signal.connect(self.show_batch_progress)
        self.detector_thread.batch_finish_signal.connect(self.show_batch_finish)
        self.detector_thread.start()
    ...
    def show_batch_progress(self, current, total):
        self.save_label.setText(f"批量进度：{current}/{total}")

    def show_batch_finish(self, msg):
